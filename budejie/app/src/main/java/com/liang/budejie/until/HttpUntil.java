...
public class HttpUntil {
                ...
                .addNetworkInterceptor(new Interceptor() {
                    ...
                    public Response intercept(Chain chain) throws IOException {
                        return chain.proceed(chain.request()).newBuilder()
                                ...
                                //cache for 30 days
                                .header("Cache-Control", "max-age=" + 3600 * 24 * 30)/*缓存30天*/
                                .build();
                    }
                })//服务器不支持缓存,使用缓存
                .cache(new Cache(context.getCacheDir(), 1024 * 1024 * 30))//服务器支持缓存Header:Cache-Control,max-age=xxx
    ...
    public static String getVideoJH(String pages, Context context) {
        return getStringByOkhttp("http://s.budejie.com/topic/list/jingxuan/41/budejie-android-6.5.8/0-" + pages + ".json?market=baidu&ver=6.5.8&visiting=&os=6.0&appname=baisibudejie&client=android&udid=352090060095762&mac=02%3A00%3A00%3A00%3A00%3A00", context);
    }
